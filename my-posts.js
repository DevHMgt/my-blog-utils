function create_posts(json) {
    ...
    for (var i = 0; i < randomposts_number; i++) {
        ...
        for (var j = 0; j < newEntry.link.length; j++) {
            if ('thr$total' in newEntry) {
                ...
                if ('media$thumbnail' in newEntry) {
                    var randompoststhumb = newEntry.media$thumbnail.url.replace(/\/s72\-c\//,'/s300/');
                } else {
        ...
        document.write('<a href="' + randompostsurl + '" rel="nofollow"><div class="post-image-wrap"><img alt="' + randompoststitle + '" src="' + randompoststhumb + '"/></div><div class="post-info"><h2 class="post-title"> '+ randompoststitle +' </h2><div class="post-meta"></div></div></a></div>');
    }
};
getValue();
for (var i = 0; i < randomposts_number; i++) {
